...
import { computeConfiguration, transformDataWithConfiguration, transformDataWithPromotions} from './searchResultsUtils';
...
export default class SearchResults extends LightningElement {
    ...
    //     let returnData = transformDataWithConfiguration(this._searchResults, this.cardConfiguration);
    //     console.log('search results normalizedSearchResults--- ', JSON.parse(JSON.stringify( returnData )));
    //     return returnData;
    // }

    @track normalizedSearchResults;
    @track isDataLoaded = false;
    _dealsCheckTriggered = false;

    async loadPromotions(hasDeals) {
        if (!this._searchResults.cardCollection) return;

        const transformed = await transformDataWithPromotions(
            this._searchResults,
            this.cardConfiguration,
            hasDeals
        );
        // console.log('with promotions --->', transformed);
        if (transformed) {
            this.isDataLoaded = true;
            this._searchResults = transformed;
            this.normalizedSearchResults = transformed;
        }
    ...
    set searchResults(value) {
        // console.log('search result setter ---', value);
        this._searchResults = value;
        // this.loadPromotions();
        //added newly
        validateDealsForAccount()
        .then(result => {
            // console.log('Result validateDealsForAccount PLP::: ', result);
            this.loadPromotions(result);
        })
    }
    get searchResults() {
     ...
     */
    get pageSize() {
        return this._searchResults?.pageSize ?? 20;
    }
     ...
     */
    get totalItemCount() {
        return this._searchResults?.total ?? 0;
    }
