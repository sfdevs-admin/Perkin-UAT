...
//fetch promotions for PLP
export async function transformDataWithPromotions( results, cardConfiguration, hasDeals ) {

    const requestId = ++latestRequestId;
    let transformedResults = transformDataWithConfiguration(results, cardConfiguration);

    // console.log('transformedResults----> ', transformedResults);
    let cardCollection = transformedResults.cardCollection || [];
    if (cardCollection.length === 0) {
        console.warn('No cards found in cardCollection.');
        // return transformedResults;
        return null;
    }

    const productIds = cardCollection.map(c => c.id);

    const webStoreResult = await fetchWebStoreId();
    const webStoreId = webStoreResult.webstoreId;
    const locale = localStorage.getItem('currentUserLocale') || 'en-US';

    let promoResults = [];
    // const promoResults =  await fetchPromotionsForProductIds(webStoreId, productIds, locale);

    const rawResponse =  await fetchPromotionsForProductIds(webStoreId, productIds, locale);

    if (requestId !== latestRequestId) {
        console.warn(`Stale promotions response ignored for requestId=${requestId}`);
        return null;
    }

    if (
        rawResponse &&
        Array.isArray(rawResponse.promotionProductEvaluationResults)
    ) {
        promoResults = rawResponse.promotionProductEvaluationResults;
    }

    const promoMap = new Map();

    promoResults.forEach(r => {
        if (!promoMap.has(r.productId)) {
            promoMap.set(r.productId, []);
        }
        promoMap.get(r.productId).push(r);
    });

    cardCollection.forEach(card => {
        const promos = promoMap.get(card.id) || [];
        if (!hasDeals) {
            // console.log('hasdeals false ::: ');
            card.b2bPromotionalPrices = promos;
        }
        // card.b2bPromotionalPrices = promos; // array of all promos for this product
    });

    // console.log('transformedResults-----> with promo--> ', transformedResults);
