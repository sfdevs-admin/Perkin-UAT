...
import { ProductCollectionAdapter } from 'commerce/productApi';
import mainTemplate from "./searchProductGrid.html";
import stencilTemplate from "./searchProductGridStencil.html";
/**
...
export default class SearchProductGrid extends LightningElement {
    ...
    productIds = [];
    fieldsList = ["Name", "B2B_Item_Class_Description__c", "B2B_Part_Number__c" , "B2B_Product_Line__c", "product_line__c"];

    @api productCollectionData;
    ...
    set displayData(data) {
        ...
        this.productIds = [];
        let cardCollection = this._displayData;
        if(cardCollection.length > 0) {
            cardCollection.forEach(product => {
                this.productIds = [...this.productIds, product.id];
            })
        }
    }
    get displayData() {
    ...
    getProductsData({ error, data }) {
        if (error) {
            ...
            if (data != undefined) {
                this.productCollectionData = data.products;
            }
        }
    }
    ...
    //     console.log('normalised disaplay data --> ', this.displayData);
    //     return this.displayData ?? [];
    // }

    get normalizedDisplayData() {
        // console.log('normalized display data --> ', this.displayData);
    ...
    @api isDataLoaded;
    render() {
        return this.isDataLoaded ? mainTemplate : stencilTemplate;
    }
