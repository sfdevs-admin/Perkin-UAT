...
export default class BuilderSearchResults extends LightningElement {
    ...
    set searchResults(value){
        console.log('BuilderSearchResults searchResults--- ', JSON.parse(JSON.stringify( value )));
    ...
    handleAddToCart(event) {
        ...
        dispatchAction(this, createCartItemAddAction(productId, quantity), {
            onSuccess: (result) => {
                console.log('#Add To Cart Success Event (PLP): ', JSON.parse(JSON.stringify(result)));
                //Add this event to Google analytics
                let productData = this.searchResults?.cardCollection.find((card) => card.id === productId);
                let product = {
                    currencyCode: result.currencyIsoCode,
                    quantity: quantity,
                    unitPrice: result.unitAdjustedPriceWithItemAdj,
                    totalPrice: result.totalPrice,
                    sku: productData.fields.B2B_Part_Number__c.value,
                    name: productData.fields.B2B_Display_Name__c.value,
                    category: this.searchResults?.categoryName,
                    brand: null
                };
