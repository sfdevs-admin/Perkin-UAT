...
//labels
import FILE_UPLOAD_SUCCESS_TEXT from '@salesforce/label/c.B2B_Checkout_FileUploadSuccess';
import UPLOAD_ATLEAST_ONE_FILE from '@salesforce/label/c.B2B_Checkout_UploadFileWarning';
import FILE_SIZE_ERROR_TEXT from '@salesforce/label/c.B2B_Checkout_FileUploadSizeText';
import TOAST_SUCCESS from '@salesforce/label/c.B2B_Toast_Success';
import TOAST_ERROR from '@salesforce/label/c.B2B_Toast_Error';
    ...
    handleUpload_lightningInput(event){
        ...
        for(let i = 0; i < files.length; i++){
            ...
            if (file.size > MAX_FILE_SIZE) {
                ...
                let message = FILE_SIZE_ERROR_TEXT
                        .replace('{0}', maxFileSize)
                        .replace('{1}', file.name)
                        .replace('{2}', fileSize);
    ...
    handleUploadFinished(files) {
        ...
        if(this.recordId && versIds.length > 0){
            createContentDocLink({versIds: versIds, encodedKey: this.currentCartId, visibleToAllUsers: this.visibleToAllUsers})
                .then((res) => {
                    this.showSuccess(FILE_UPLOAD_SUCCESS_TEXT,versIds);
                })
                .catch(error => {
    ...
    validate(){
        if(this.docIds.length === 0 && this.required === true){ 
            ...
            if(this.requiredMessage == null){
                errorMessage = UPLOAD_ATLEAST_ONE_FILE;
            }
            else{
